...
test('Kitajiri kimono hero source reconstructs to a valid production WebP', async () => {
  const image = await decodeHeroSource();
  assert.ok(image.length > 15000, `reconstructed kimono hero is unexpectedly small: ${image.length} bytes`);
  ...
  assert.ok(width >= 360, `reconstructed hero width is too small: ${width}px`);
  assert.ok(height >= 450, `reconstructed hero height is too small: ${height}px`);
